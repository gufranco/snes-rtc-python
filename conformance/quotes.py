...
referenceDoes quotes the recording this project is measured against, which is
not in a pinned document, so looking for it there would report an absence that
means nothing.
"""
...
def unpaged(records: Iterable[tuple[str, Any]] | None = None) -> list[str]:
    ...
    The ones in `PAGELESS` are already known and already written down, so they
    are not reported again. Anything else is new and fails.
    """
    found: list[str] = []
    for name, held in loaded() if records is None else records:
        for where in uncited(held, False, name):
            if any(one in where for one in ELSEWHERE) or where in PAGELESS:
                continue
            found.append(where)
    return found
