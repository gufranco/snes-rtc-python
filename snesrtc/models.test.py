...
class CatalogueTest(unittest.TestCase):
    def test_the_catalogue_covers_the_sharp_part(self):
        self.assertIn("s-rtc", models.MODELS)

    def test_and_the_epson_part_the_other_cartridges_carried(self):
        self.assertIn("rtc-4513", models.MODELS)

    def test_every_model_says_what_it_is_and_where_it_answers(self):
...
class BuildTest(unittest.TestCase):
    def test_the_sharp_model_builds_the_sharp_protocol(self):
        built = models.describe("s-rtc").build()

        self.assertIsInstance(built, sharp.Clock)

    def test_the_epson_model_builds_the_other_one(self):
        built = models.describe("rtc-4513").build()

        self.assertIsInstance(built, epson.Clock)

    def test_a_built_clock_gets_a_store_of_its_own_when_none_is_given(self):
        built = models.describe("s-rtc").build()

        self.assertIsInstance(built.store, store.Store)

    def test_and_uses_the_one_it_is_given_when_there_is_one(self):
    ...
    def test_a_built_clock_can_be_handed_the_clock_it_reads(self):
        built = models.describe("rtc-4513").build(now=lambda: 42)

        self.assertEqual(built.now(), 42)

    def test_a_built_clock_knows_which_model_it_is(self):
