...
class ReadTest(unittest.TestCase):
    def test_a_read_before_anything_answers_the_ready_marker(self):
        chip = clock()

        self.assertEqual(chip.read(sharp.DATA), 0x0F)

    def test_the_thirteen_bytes_then_come_out_in_order(self):
        chip = clock()
    ...
    def test_and_the_marker_comes_round_again_at_the_end(self):
        chip = clock()
        chip.read(sharp.DATA)
        for _ in range(13):
            chip.read(sharp.DATA)
    ...
    def test_the_sequence_then_starts_over(self):
        chip = clock()
        for _ in range(15):
            chip.read(sharp.DATA)
    ...
    def test_a_read_while_not_in_reading_mode_answers_nothing(self):
        chip = clock()
        chip.write(sharp.CONTROL, 0x0E)

        self.assertEqual(chip.read(sharp.DATA), 0x00)

    def test_a_read_of_an_address_the_chip_does_not_answer_gives_the_open_bus(self):
        chip = clock()
        chip.open_bus = 0x42
...
class WriteTest(unittest.TestCase):
    def test_a_written_time_reads_back_as_it_was_written(self):
        chip = clock()
        set_time(chip, DATE)
    ...
    def test_the_weekday_is_worked_out_rather_than_written(self):
        chip = clock()
        set_time(chip, DATE)
    ...
    def test_only_the_low_four_bits_of_a_written_byte_are_kept(self):
        chip = clock()
        chip.write(sharp.CONTROL, 0x0E)
        chip.write(sharp.CONTROL, 0x00)
    ...
    def test_writing_past_the_twelfth_byte_is_ignored(self):
        chip = clock()
        set_time(chip, DATE)
    ...
    def test_a_write_outside_writing_mode_changes_nothing(self):
        chip = clock()
    ...
    def test_a_write_to_an_address_the_chip_does_not_answer_changes_nothing(self):
        chip = clock()
        before = list(chip.store.bytes)
...
class CommandTest(unittest.TestCase):
    def test_the_reset_command_clears_the_thirteen_time_bytes(self):
        chip = clock()
        set_time(chip, DATE)
    ...
    def test_a_command_the_chip_does_not_know_leaves_it_idle(self):
        chip = clock()
    ...
    def test_the_marker_command_is_accepted_and_does_nothing(self):
        chip = clock()
        before = list(chip.store.bytes)
    ...
    def test_the_read_command_puts_the_chip_back_at_the_start(self):
        chip = clock()
        chip.read(sharp.DATA)
        chip.read(sharp.DATA)
...
class TimeTest(unittest.TestCase):
    def test_a_clock_read_later_has_moved_forward(self):
        held = store.Store(cleared=True)
        held.stamp = FROZEN
        moment = [FROZEN]
    ...
    def test_a_clock_read_at_the_same_moment_has_not(self):
        chip = clock()
        set_time(chip, DATE)
        chip.write(sharp.CONTROL, 0x0D)
        chip.read(sharp.DATA)
    ...
    def test_a_stamp_from_the_future_is_treated_as_no_time_having_passed(self):
        chip = clock()
        set_time(chip, DATE)
    ...
    def test_a_stamp_that_wrapped_past_the_top_is_measured_the_long_way_round(self):
        chip = clock(at=10)
        set_time(chip, DATE)
    ...
    def test_the_reading_advances_the_stamp_to_now(self):
        chip = clock()
...
class ResetTest(unittest.TestCase):
    def test_a_reset_leaves_the_chip_ready_to_be_read(self):
        chip = clock()
        chip.write(sharp.CONTROL, 0x0E)
    ...
    def test_a_reset_does_not_clear_what_the_battery_kept(self):
        chip = clock()
        set_time(chip, DATE)
    ...
    def test_a_chip_prints_as_its_mode_and_its_place_in_the_sequence(self):
