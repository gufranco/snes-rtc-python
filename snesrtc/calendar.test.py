...
class LeapTest(unittest.TestCase):
    def test_a_year_divisible_by_four_is_long(self):
        self.assertTrue(calendar.is_leap(2024))

    def test_a_century_is_not(self):
        self.assertFalse(calendar.is_leap(1900))

    def test_but_every_fourth_century_is(self):
        self.assertTrue(calendar.is_leap(2000))

    def test_an_ordinary_year_is_not(self):
        self.assertFalse(calendar.is_leap(2023))


class LengthTest(unittest.TestCase):
    def test_the_months_are_the_lengths_everyone_knows(self):
        self.assertEqual(
            [calendar.days_in(2023, month) for month in range(1, 13)],
            [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31],
        )

    def test_february_gains_a_day_in_a_long_year(self):
        self.assertEqual(calendar.days_in(2024, 2), 29)

    def test_a_month_number_past_the_end_wraps_rather_than_failing(self):
        self.assertEqual(calendar.days_in(2023, 13), 31)


class WeekdayTest(unittest.TestCase):
    def test_the_epoch_the_chip_counts_from_was_a_monday(self):
        self.assertEqual(calendar.weekday(1900, 1, 1), 1)

    def test_a_date_whose_weekday_is_a_matter_of_record(self):
        self.assertEqual(calendar.weekday(2008, 1, 1), 2)

    def test_the_day_after_saturday_is_sunday(self):
        self.assertEqual(calendar.weekday(2026, 8, 15), 6)
        self.assertEqual(calendar.weekday(2026, 8, 16), 0)

    def test_a_year_before_the_epoch_is_pulled_forward_to_it(self):
        self.assertEqual(calendar.weekday(1800, 1, 1), calendar.weekday(1900, 1, 1))

    def test_a_month_outside_the_year_is_pulled_inside_it(self):
    ...
    def test_a_day_outside_the_month_is_pulled_inside_it(self):
...
class AdvanceTest(unittest.TestCase):
    ...
    def test_a_long_jump_lands_where_the_calendar_says(self):
    ...
    def test_two_moments_holding_the_same_reading_are_interchangeable(self):
        first = calendar.Moment(2026, 8, 18, 12, 0, 0, 2)
        second = calendar.Moment(2026, 8, 18, 12, 0, 0, 2)

        self.assertEqual({first, second}, {first})

    def test_a_moment_reports_itself_as_something_readable(self):
